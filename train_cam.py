...
from gan_training.train_cam import Trainer, update_average
...
def init_cam_weights(model, target, cur_task_id, rel_task_id=-1):
    if rel_task_id == -1:
        return
    for module in model.children():
        if module.__class__.__name__ == target:
            for name, param in module.named_parameters():
                if name.find("cam") >= 0 and name.find(f".{cur_task_id - 1}.") >= 0 and name.find('weight') >= 0:
                    attr_name = name.split('.')[0]
                    getattr(module, attr_name)[cur_task_id - 1].weight = nn.Parameter(
                        getattr(module, attr_name)[rel_task_id - 1].weight.data)
                    try:
                        getattr(module, attr_name)[cur_task_id - 1].bias = nn.Parameter(
