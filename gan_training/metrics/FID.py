...
def extract_features(imgs, inception, device):

    feature_list = []

    for img_batch in imgs:
        img_batch = img_batch.to(device)
        feature = inception(img_batch)[0].view(img_batch.shape[0], -1)
...
def FID(imgs, task_id, config=None, device=None, batch_size=32, resize=False, splits=1):
    """
    Computes the frechet inception distance of the generated images imgs
        Args:
    ...
    if "joint" in config['data']['train_dir']:
        inception_path = os.path.join(base_path, f"cl/inception_rec/task_{task_id+1}.pkl")
    else:
        inception_path = os.path.join(base_path, f"cl/inception_rec/task_{task_id}.pkl")
