...
class Gen_ResnetBlock(nn.Module):
    ...
    def forward(self, x, y):
        task_id = y[0]-1 # Pretrain-based (1, 2, ..., n_task)
        x_s = self._shortcut(x)
        dx = self.conv_0(actvn(x))
        if task_id >= 0:
            gx = self.cam_gconv_0[task_id](actvn(dx))
            px = self.cam_pconv_0[task_id](actvn(dx))
            rx = self.cam_rconv_0[task_id](actvn(x))
            x = gx+px+rx
        else:
            x = dx
        dx = self.conv_1(actvn(x))
        if task_id >= 0:
            gx = self.cam_gconv_1[task_id](actvn(dx))
            px = self.cam_pconv_1[task_id](actvn(dx))
            rx = self.cam_rconv_1[task_id](actvn(x))
            dx = gx+px+rx
        out = x_s + 0.1*dx
