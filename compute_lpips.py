...
def calc_lpips(args, fn):
    ...
    with torch.no_grad():
        ...
        res = dict()
        for dir in dir_list:
            filelist = os.listdir(os.path.join(args.path, dir))
            ...
            for i in tqdm(range(N)):
                for j in range(i + 1, N):
                    input1_path = os.path.join(args.path, dir, filelist[i])
                    input2_path = os.path.join(args.path, dir, filelist[j])
...
if __name__ == '__main__':
    ...
    res = calc_lpips(args, fn)

    # print(f"LPIPS: {res:.3f}")
    print(res)
    #
